library(bslib)
library(cachem)
library(dplyr)
library(modules)
library(shiny)
library(markdown)

tableTab <- use("modules/mod-table.R")
trendTab <- use("modules/mod-trend.R")
mapTab <- use("modules/mod-map.R")
consts <- modules::use("modules/constants.R")

ui <- page_navbar(
 title = consts$app_title,
 id = "navbar",
 collapsible = TRUE,
 fillable = TRUE,
 fillable_mobile = TRUE,
 footer = consts$rcp_legal,
 theme = bslib::bs_theme(
  version = 5,
  bootswatch = "minty",
  bg = "#FFFFFF",
  fg = "#307972",
  primary = "#44AD99",
  secondary = "#AAAAAA",
  base_font = font_google("Poppins", local = TRUE)
 ) %>%
  bs_add_variables(
   "btn-active-color" = "#FFBC00 !important;"
  ) %>%
  bs_add_rules(
   list(
    "
   .optgroup-header {
   font-size: 1.125rem !important;
   color: #AAAAAA !important;
   }
   .btn-secondary:hover {
   color: $btn-active-color
   }
   .line {
    background-color: #59948E !important;
}
   ")
   ),
 nav_panel("Overview",
           tableTab$tableUI("table"),
           icon = bsicons::bs_icon("house-fill")
 ),
 nav_panel("Trend",
           trendTab$trendUI("trend"),
           icon = bsicons::bs_icon("graph-up-arrow")
 ),
 nav_panel("Geographic Insights",
           mapTab$mapUI("map"),
           icon = bsicons::bs_icon("geo-fill")
 )
)

server <- function(input, output, session) {

 # Home tab server ----
 session$userData$tableTab <- tableTab$tableServer(
  id = "table",
  df1 = consts$ctab_stats %>% collect(),
  df2 = consts$ftab_stats %>% collect(),
  df3 = consts$ttip
  )

 # Lineplot tab server ----
 session$userData$trendTab <- trendTab$trendServer(
  id = "trend",
  df1 = consts$metrics_lbl,
  df2 = consts$cyearly_stats,
  df3 = consts$fyearly_stats,
  df4 = consts$chr_stats,
  df5 = consts$fhr_stats,
  df6 = consts$chosp_stats,
  df7 = consts$fhosp_stats,
  df8 = consts$ttip
  )

 # Map tab server ----
 session$userData$mapTab <- mapTab$mapServer(
  id = "map",
  df1 = consts$metrics_lbl,
  df2 = consts$ttip,
  df3 = consts$geo_lbl,
  df4 = consts$ccd_stats,
  df5 = consts$ccl_stats,
  df6 = consts$cchn_stats,
  df7 = consts$chr_stats,
  df8 = consts$cd_shp,
  df9 = consts$cl_shp,
  df10 = consts$chn_shp,
  df11 = consts$hr_shp,
  df12 = consts$fcd_stats,
  df13 = consts$fcl_stats,
  df14 = consts$fchn_stats,
  df15 = consts$fhr_stats,
  df16 = consts$hosp_shp
  )

}

# Run the app
shinyApp(ui, server)

